...
const Certificado = {
  ...
  update: async (id, data) => {
    const { id_producto, codigo, descripcion, fecha_emision, fecha_vencimiento, estado, usuario_modificacion } = data;
    const query = `
      UPDATE certificado
    ...
    const values = [id_producto, codigo, descripcion, fecha_emision, fecha_vencimiento, estado, usuario_modificacion, id];
    const result = await pool.query(query, values);
    return result.rows[0];
  },
